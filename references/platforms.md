...
For confirmed playlist creation or adding after recommendations, use Spotify Desktop controls only when visible and reliable. Read `playlist-computer-use.md` first, then either target the user-selected existing playlist or create the requested new playlist and name it. Search each agreed recommendation by artist-title in the playlist search field, verify the current `Top` result row, add it once, and confirm the playlist count increased by exactly one. Do not use the `Recommended / Based on what's in this playlist` block for requested additions. Skip ambiguous or stale search results. Do not use Spotify Web Player.
...
For confirmed playlist creation or adding after recommendations, use Music.app or Apple Music desktop controls only when the requested existing/new playlist target and add-to-playlist flow are visible and reliable. Apple may require catalog/library availability for adding tracks; skip tracks that cannot be confidently found or added.
...
For confirmed playlist creation or adding after recommendations, use Amazon Music desktop controls only when the requested existing/new playlist target and add controls are available in the current account/app state. Amazon Music UI varies by plan and region; if the flow is unavailable or results are ambiguous, skip playlist changes and report the limitation.
...
For confirmed playlist creation or adding after recommendations, apply the same rule: use the requested existing/new playlist target through the installed client only when the controls are clear and reliable; otherwise provide the recommendations without modifying the account.
