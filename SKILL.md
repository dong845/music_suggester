...
Build recommendations from evidence, not vibes alone. Enter the user's authorized installed music app or connected music account, collect music-library data, derive a durable taste profile, compare it with any previous profile, then recommend items that are novel, explainable, and deduplicated. Do not stop to ask preference-brief questions before recommending; use any intention, mood, language, era, artist, discovery constraints, or requested recommendation count the user already provided, otherwise infer a balanced recommendation set from the taste profile. After the taste analysis, include a short non-blocking set of questions the user can answer to refine the next list, such as music type, era/time age, country or language, specific musicians, and how many songs they want; then continue directly into recommendations without waiting for answers. After recommendations, check whether the chosen desktop app appears to support playlist creation and adding tracks, report that availability, and leave the recommended list visible for the user to review. End the recommendation response by asking whether the user wants the recommendations added by computer-use, and whether they want an existing playlist or a new playlist. Create a playlist or add recommendations only after the user explicitly agrees in a follow-up; do not add tracks during the initial recommendation response. Keep the workflow platform-neutral: Spotify, Apple Music, Amazon Music, NetEaseMusic, YouTube Music desktop apps, TIDAL, Deezer, SoundCloud, and other downloaded music clients are valid sources when installed and authorized.
...
1. Identify automatic access paths.
   ...
   - Prefer opening or attaching to an installed desktop app such as Spotify Desktop, Apple Music/Music.app, Amazon Music, NetEaseMusic, YouTube Music as a standalone installed app only if it can be controlled without Chrome, TIDAL, Deezer, SoundCloud, Last.fm desktop tooling, or another named downloaded music app using the user's existing logged-in session.
   ...
   - If the first-choice app is unavailable, inspect available downloaded apps/plugins/connectors for Spotify Desktop, Apple Music/Music.app, Amazon Music, NetEaseMusic, YouTube Music installed app, TIDAL, Deezer, SoundCloud, Last.fm, ListenBrainz, MusicBrainz, or generic desktop-app automation.
...
2. Capture stated recommendation intent without asking an interview.
   ...
   - If the user already provided constraints such as mood, language, region, era, artist focus, singer type, discovery posture, use case, platform, or number of songs to recommend, treat them as current-session intent.
   ...
   - If the user did not specify a recommendation count, use the default list size and ask the count question later in `Questions to refine the next list`; do not block the current recommendations just to collect a number.
...
4. Enter the app and collect or refresh library evidence.
   - Open or attach to the relevant downloaded music app: Spotify Desktop, Apple Music/Music.app, Amazon Music, NetEaseMusic, YouTube Music installed app, TIDAL, Deezer, SoundCloud, Last.fm desktop tooling, or another installed platform the user names.
   ...
   - If desktop UI automation exposes only a sparse accessibility tree, or if scrolling the whole library would be slow or fragile, inspect the installed app's local read-only cache/database as an additional automatic source before declaring full extraction impossible. Look for table pairs or files that separate collection membership from metadata, such as playlist or liked-track ID lists joined to track metadata records. Do not read cookies, session tokens, auth secrets, payment/account data, or unrelated local storage. Use only music-library metadata needed for analysis and dedupe.
   - Do not assume an obvious table name is authoritative. If a table such as `track`, `songs`, or `library` is empty, continue checking nearby tables for `playlist`, `favorite`, `liked`, `history`, `playing`, `dbTrack`, `trackIds`, `metadata`, or platform-specific names. When a full collection can be reconstructed by joining IDs to metadata, treat that as stronger evidence than a small visible UI sample, and record the extraction path plus cache freshness timestamp.
...
6. Analyze taste across multiple axes.
   ...
   - Current intent: any mood/atmosphere, language/region, era/time age, singer/artist focus, discovery posture, use case, and desired recommendation count explicitly provided by the user. If none was provided, say that ranking uses balanced taste-profile fit.
...
7. Recommend with dedupe and explanation.
   ...
   - Size the final recommendation list from the user's requested count when provided. Otherwise default to 10-20 recommendations. If the requested count is impractically large for the available evidence or desktop-app workflow, provide a manageable first batch and state the limit clearly.
...
8. Offer optional refinement questions.
   - Immediately after `Taste analysis`, include a section named `Questions to refine the next list`, then continue to the recommendation sections in the same response. If there is no usable library evidence and no `Taste analysis` section, place these questions after the lower-confidence evidence note and before recommendations.
   ...
   - Ask concise, optional questions that would materially change the next recommendation round: preferred music type/genre, era or time age, country/region/language, mood/use case, discovery level, whether to focus on or avoid specific musicians, and how many songs the user wants recommended.
...
9. Check playlist availability and create only after consent.
   ...
   - End the recommendation response with a direct consent question: ask whether the user wants Codex to use computer-use in the installed desktop app to add the recommendations, and ask them to choose either an existing playlist or a new playlist. If playlist availability is unavailable, ambiguous, or blocked, ask only whether they want manual instructions or a retry after fixing the app state.
   ...
   - If the user confirms adding but does not specify an existing playlist or a new playlist, ask that one target question before modifying the app. For an existing playlist, ask for or identify the target playlist name; for a new playlist, ask for a name only when the user wants to choose one, otherwise use the default dated name.
   ...
   - If the user confirms adding recommendations, ask whether to add all recommendations or only selected items when their intent is unclear. If the user says to add all, add only the final deduped recommendation set, respecting any requested song count, after checking it against liked songs, saved tracks, owned playlists, saved playlists, and the target playlist contents visible in the app. Search by artist-title, verify the matched track/album when possible, skip uncertain matches rather than adding the wrong item or an existing library item, and report skipped/ambiguous items.
   - For any computer-use playlist creation or add step, read `references/playlist-computer-use.md` first and follow its transaction loop. Treat each song add as a guarded operation: clear stale search state, search the current song, verify a row match, perform one add action, then verify the target playlist changed by exactly one expected track before continuing. For apps such as NetEaseMusic with sparse accessibility trees, follow the sparse accessibility / visual UI protocol in that reference before deciding the app is unusable.
...
10. Save state.
   ...
   - Save any user-provided current-session intent, including requested recommendation count, with the run summary, but do not treat it as permanent taste unless it recurs across sessions.
...
- `Playlist action?`: required as the final section of the initial recommendation response; ask whether to add the recommendations with computer-use and whether to use an existing playlist or create a new playlist. If the app cannot support reliable playlist changes, ask whether the user wants manual instructions or a later retry instead.
...
- `Questions to refine the next list`: required immediately after `Taste analysis` and before recommendation sections, or after the lower-confidence evidence note when no taste analysis is possible; ask optional follow-up questions about music type/genre, era/time age, country/region/language, mood/use case, discovery level, specific musicians to focus on or avoid, and the desired number of songs.

When evidence is thin or empty, say so and ask for one additional source only after giving a small starter set.

## Intent Handling

Do not ask a preference brief before recommending. Extract intent only from the user's request, such as "Japanese R&B", "workout", "new releases", "female vocals", "similar to X", "avoid Y", "more obscure", or "recommend 15 songs". If no intent or song count is stated, proceed with balanced recommendations from the taste profile using the default size and mention that no extra preference constraints were provided. After the taste analysis or lower-confidence evidence note, list optional refinement questions, including how many songs the user wants next time, so the user can steer the next list, then give recommendations in the same response without waiting.
...
Read `references/playlist-computer-use.md` before creating or adding to playlists through desktop-app UI automation.
